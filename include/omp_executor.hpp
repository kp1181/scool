...
namespace scool {
    ...
    public:
        ...
        omp_executor() : ctx_(*this) {
            ...
            #pragma omp single
            {
                ...
                this->log().info(this->NAME_) << "ready with " << p_ << " threads" << std::endl;
            }
        } // omp_executor
    ...
    private:
        ...
        void m_process__() {
            ...
            #pragma omp parallel default(none) shared( curr_, ctx_, sts, next_, std::cout, S_, m_state)
            #pragma omp single 
            for (int b = 0; b < B_; b++) {
                if (m_state[b] == true) {
            #pragma omp task
